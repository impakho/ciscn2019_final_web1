...
<div class="container" style="margin-top:30px">
    <h3 style="margin-bottom:15px;">上传音乐</h3>
    ...
    <script>
        ...
        $("#upload").on("fileuploaded", function (event, data, previewId, index) {
            ...
            if (data.status==1){
                ...
                try{
                    data.info+="<br>音乐名："+decodeURIComponent(escape(atob(data.title)));
                    data.info+="<br>歌手："+decodeURIComponent(escape(atob(data.artist)));
                    data.info+="<br>专辑："+decodeURIComponent(escape(atob(data.album)));
                }catch(err){
                    data.info+="<br>音乐名："+atob(data.title);
                    data.info+="<br>歌手："+atob(data.artist);
                    data.info+="<br>专辑："+atob(data.album);
                }
            }else{
